...
TOURNAMENT_SIZE = 5      # numero di individui scelti a caso nel processo di selezione
...
FUNCTIONS = [add, sub, mul]          #Non terminal set
TERMINALS = ['x', -2, -1, 0, 1, 2]   #terminal set

'''
    Funzione a cui vogliamo giungere. Nel dataset ci sono soltanto i punti in base ai quali l'algoritmo di GP
    andrà a tirar fuori questa formula.
'''
def target_func(x): # evolution's target
    return x*x*x*x + x*x*x + x*x + x + 1

'''
    Sulla target function genera una serie di punti equidistanziati, per generare una dataset di punti
    sui quali costruire la formula.
'''
...
'''
    Calcolo della fitness tenendo conto della funzione di errore.
    Se l'errore è > 1 darà un valore abbastanza piccolo.
'''
...
def main():      
    # init stuff
    seed(319) # init internal state of random number generator
